...
class Matrix:
    ...
    def update(self):
            ...
            while heap:
                ...
                if ((k not in self.mpv) and (d > 0) and (d < 20)): self.mpv.add(k)
            ...
            tl = list(self.e.keys())
            random.shuffle(tl)
            for a in tl: self.e[a] = {key:(value - 1) for key, value in self.e[a].items() if (value > 0)}
