...
class Sensorium:
    def __init__(self, po_in):
        ...
        self.sv = set()
        ts_len = 5#---------------------------------------------------------------------------------------------------------------HP
        self.aff_ch_A_card = round(float(aff_dim) / float(ts_len + 1))
        tv = self.aff_v.copy()
        self.ts = []
        ...
        self.eff_card = round(float(eff_dim) / float(self.beh_map_max_size + 1))
        tv = self.eff_v.copy()
        self.beh_map = dict()
        ...
        self.sv_card = (em_int_card + self.aff_ch_A_card + self.eff_card)
        self.bv = set()
    def update(self):
        self.bv = self.po.m[0].pv.copy()
        self.bv &= self.eff_v
...
class Matrix:
    def __init__(self, po_in, mi_in):
        ...
        start_idx = -round((self.po.dim - 1) / 2)
        ei = {(start_idx + a) for a in range(self.po.dim)}
        self.cv_dim = 3
        cd = {(start_idx + a):0 for a in range((self.po.dim * self.cv_dim))}
    ...
    def update(self):
            ...
            self.em = (float(len(self.pev)) / float(max(1, (len(iv) + len(self.pv)))))
            self.process_EPIFOR()
            wd = 1
            for a in self.cv:
            ...
            if self.cv_dim == 3: self.cv = (iv | {(self.po.dim + a) for a in self.pv} | {((self.po.dim * 2) + a) for a in fbv})
            if self.cv_dim == 2: self.cv = (iv | {(self.po.dim + a) for a in fbv})
