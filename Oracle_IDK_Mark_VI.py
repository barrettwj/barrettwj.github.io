...
class Matrix:
    ...
    def update(self):
        ...
        #     fbv = {random.choice(range((a * self.po.M), ((a + 1) * self.po.M))) for a in rs}
        #     # fbv = set()
        ##############################################################################################################
        cv = (self.pv | {-(a + 1) for a in fbv})
        ...
        if le > 0:
            ...
            for a in aks:
                ...
                if cli in vi:
                    vi[cli][1].add(a)
                    #########################################
                    rel_v = {-(b + 1) for b in (set(range((cli * self.po.M), ((cli + 1) * self.po.M))) & fbv)}
                    if a in self.e:
        ...
        for a in iv:
            ...
            if len(ovl) == 0:
                ...
                wi = ovl.pop()
            if wi in self.e:
                for b in pvc: self.e[wi][b] = random.randrange(self.po.adc_min, self.po.adc_max)
            else: self.e[wi] = {b:random.randrange(self.po.adc_min, self.po.adc_max) for b in pvc}
            self.pv.add(wi)
        ...
        self.forget_period_ct = 0#---------------------------------------------------------------DEBUG!!!
        if self.forget_period_ct == self.forget_period:
            tl = list(self.e.keys())
            for a in tl:
                if len(self.e[a]) > 1:#---------------------------------------------------------------------HP
