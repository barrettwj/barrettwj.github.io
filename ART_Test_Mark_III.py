...
blank_v = [mid_val] * N
thresh = 0.01#-0.01---------------------------------------------------------------------------------------------HP
sp = 200#-proportional to abs_delta_range???--------------------------------------------------------------------HP
lrA = 0.001#-0.001----------------------------------------------------------------------------------------------HP
lrB = 1#-0.80---------------------------------------------------------------------------------------------------HP
adc_max = 1000000#----------------------------------------------------------------------------------------------HP
forget_period = 10#---------------------------------------------------------------------------------------------HP
...
# ts = [[random.randint(min_val, max_val) for _ in range(Ns)] for _ in range(ts_dim)]
###########################################################################################
...
while True:
    ...
    while ((len(B) > 0) and (mv > thresh_mod)):
        ...
        mem[mk][1][0] = 0
        comp.add(mk)
        B = [(a[0], (a[1] - (1 - A[mk]))) for a in B if (a[0] not in comp)]
        lb = len(B)
        if (lb == 0):
            rA += 1
            avail_idcs = (mem_potential_idcs - mem.keys())
        ...
        for i, a in enumerate(exiv):
            ...
            for b in comp:
                ...
                if (diff > 0):
                    ...
                    val_B = (mem[b][0][i] + (signB * diff * lrB))
                    if val_B < min_val: val_B = min_val
                    if val_B > max_val: val_B = max_val
                    mem[b][0][i] = val_B
    #############################################################################################################################
    mem_copy = mem.copy()
    for kA, vA in mem_copy.items():
        if (mkt_p in vA[2]) and (kA != mk): del mem[kA][2][mkt_p]
    ###########################################################################################
    ...
    # mk_p = set(p_dict.keys()).pop() if p_dict else -1
    mk_p = set(p_dict.keys()).pop() if (len(p_dict) == 1) else -1
    # mk_p = set(p_dict.keys()).pop() if (len(p_dict) > 0) else -1
    ...
    #############################################################################################################################
    print(" | ".join([
        f"ID:{str(ts_idx).rjust(3)}", f"PV:{str(mk_p).rjust(4)}", f"MK:{str(mk).rjust(4)}", f"MV:{f'{mv / norm:.4f}'.rjust(7)}",
        f"CO:{f'{avg_corr:.2f}'.rjust(4)}", f"DU:{f'{avg_dups:.2f}'.rjust(4)}", f"ML:{str(len(mem)).rjust(3)}",
        f"SK:{str(len(comp)).rjust(3)}", f"RA:{str(rA).rjust(3)}", f"RB:{str(rB).rjust(3)}", f"RC:{str(rC).rjust(3)}",
        f"RD:{str(rD).rjust(3)}", f"PE:{pes.rjust(3)}", f"ATCD:{f'{avg_tcd:.2f}'.rjust(4)}", f"EXP:{str(exp).rjust(4)}"
    ]))
    #############################################################################################################################
    # forget_period_ct += 1
