...
class Matrix:
    ...
    def update(self):
        ...
        self.Bv = (self.iv | {(self.po.K + a) for a in self.pv} | {((self.po.K * 2) + a) for a in fbv})
        # self.Bv = (self.iv | {(self.po.K + a) for a in fbv})
        # self.Bv = (self.pv | {(self.po.K + a) for a in fbv})#----------why doesn't this work????
        #_____________________________________________________________________________________________________________________________
        avail_indices = (self.poss_indices - set(self.mem.keys()))
        self.mem[random.choice(list(avail_indices))] = [self.Bv.copy(), self.blank_cv.copy(),
        ...
        print(f"M{self.mi}\tER: {erm:.2f}%\tTP: {self.tp}\tMEM: {len(self.mem)}" + agency_str)
