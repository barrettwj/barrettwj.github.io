...
class Oracle:
    def __init__(self):
        ...
        self.D = self.N * 2
        self.D_range = [-self.N + a for a in range(self.D)]
        self.M = 47#--------------------------------------------------------------------------------------------HP
...
class Matrix:
    ...
    def update(self):
        ...
        if self.ffi == -1:
            ...
            # bv = self.pv.copy()
            bv = self.bv_prev.copy()
            self.bv_prev = self.pv.copy()
            ...
            self.ff = {-(a + 1) for a in self.ff if (a // self.po.M) in self.po.N_range}
            self.ff |= self.po.ts[ts_idx]
        ...
        remn = set(self.po.D_range) - self.convert(self.pv)
        thresh_A = 0.90#-------------------------------------------------------------------------------------HP
        maxval = 1
        avgd = 0
        avgd_ct = 0
        ...
        tl.append(f"PV: {str(len(self.pv)).rjust(4)}")
