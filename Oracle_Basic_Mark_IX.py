...
class Oracle:
    def __init__(self):
        ...
        ###################################################################
        self.bv_set = {a for a in range(self.ts_dim)}
        self.ext_map = dict()
    ...
    def update(self):
        if self.ffi == -1:
            ...
            if self.ts_idx_inc == -1:
                self.ts_idx_inc = random.choice(list(self.po.bv_set))
        ...
        # self.forget_ct += 1
        self.forget_ct = 0
        if self.forget_ct == self.forget_period:
            for kA in self.e.keys():
                for kB in self.e[kA].keys():
                    if (self.e[kA][kB] < self.adc_max): self.e[kA][kB] += 1
            thresh_A = 100#-----------------------------------------------------------------------------------------HP
            for kA in self.e.keys(): self.e[kA] = {kB:vB for kB, vB in self.e[kA].items() if vB < thresh_A}
            self.e = {k:v for k, v in self.e.items() if v}
            self.forget_ct = 0
        ###############################################################################################################################
        self.cv = av.copy()#-----------------------------------THIS SEEMS TO WORK BEST!!!
        ######################################################################################
        self.pv = er_el.copy()
        thresh_B = 0.99#-----------------------------------------------------------------------------------------------HP
        maxval = 1
        ...
        for a in range(self.po.N):
            ...
            if b:
                ...
                # c = {k:(1 - (len(self.e[k].keys() ^ self.cv) / self.po.Q)) for k in b}#-Hamming distance
                ...
                avgs += m
                avgs_ct += 1
                if m >= thresh_B: self.pv.add(d[0][1])
        avgd /= max(1, avgd_ct)
        avgs /= max(1, avgs_ct)
        ###############################################################################################################################
        ...
        tl.append(f"MEM: {str(len(self.e)).rjust(8)}")
        ...
        tl.append(f"BL: {str(len(self.po.ext_map)).rjust(3)}")
        if self.ffi == -1: tl.append(f"BI: {str(self.ts_idx_inc).rjust(3)}")
        print(" | ".join(tl))
        ###############################################################################################################################
