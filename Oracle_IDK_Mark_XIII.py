...
class Oracle:
    def __init__(self):
        ...
        self.ts_map_vecs = [v for v in self.ts_map.values()]
        self.ts_map_vecs_idx = 0
        self.m = [Matrix(self, a) for a in range(self.H)]
    def update(self):
        while True:
            for a in self.m: a.update()
            self.ts_map_vecs_idx = ((self.ts_map_vecs_idx + 1) % self.ts_dim)
class Matrix:
    def __init__(self, po_in, mi_in):
        ...
        self.adc_max = 1000#-resistance to replacement by creative inception---------------------------------HP
    ...
    def update(self):
        ...
        if (self.ffi == -1):
            ...
            self.iv = ds[0][2].copy()
            self.iv |= ds[0][1]
            # self.iv = self.po.ts_map_vecs[self.po.ts_map_vecs_idx].copy()
            # self.iv |= exov
        ...
        wd = 1#------------------------------------------------------------------------------------------HP
        for a in self.sel_v:
            # mult = ((self.max_dist_found + 1) - a[1])#--------------------------------------------??????
            # mult = (2.0 - a[1])#--------------------------------------------------------------------??????
            mult = 1
            for j, b in enumerate(self.mem[a[0]][0]):
                if (j in self.iv): self.mem[a[0]][0][j] = min(self.dv_mag_max, (b + (wd * mult)))
                else: self.mem[a[0]][0][j] = max(self.dv_mag_min, (b - (wd * mult)))
        ####################################################################################################
        # diff = (len(self.mem) - self.mem_cap_max)
        # if (diff > 0):
        #     self.mem = {k:[v[0], (v[1] - 1)] for k, v in self.mem.items() if (v[1] > 0)}
        #     d = [(v[1], k) for k, v in self.mem.items()]
        #     rs = random.sample(range(len(d)), len(d))
        #     ds = sorted(d, key = lambda x: (x[0], rs.pop()))
        #     for i in range(diff): del self.mem[ds[i][1]]
        ####################################################################################################
        self.process_inference()
    ...
    def process_inference(self):
        ...
        self.cv = (self.iv | fbv_mod | self.pv)
        # ts = (self.pv ^ fbv)
        # self.cv = (self.iv | ts)
        self.sel_v = self.generate_sel_v(self.cv)
        sum_v = self.blank_dv.copy()
        for a in self.sel_v: sum_v = [(x + y) for x, y in zip(sum_v, self.mem[a[0]][0])]
        ...
        while ((len(out) < sample_dim) and (num_attempts < num_attempts_max)):
            ...
            while ((len(exc_ks) < len(sel_prime)) and (len(out) < sample_dim)):
                ...
                if (valA <= vp):
                    ...
                    if len(diff_v) > 0:
                        ...
                        wk = frozenset(wkA)
                        self.mem[wk] = [self.mem[ds[0][0]][0].copy(), self.adc_max]
                        self.num_gen += 1
                        del self.mem[ds[0][0]]
                    else: self.mem[ds[0][0]][1] = self.adc_max
            ...
            if not vp_met:
                ...
                if wk not in self.mem.keys():
                    self.mem[wk] = [self.blank_dv.copy(), self.adc_max]
                    self.num_gen += 1
